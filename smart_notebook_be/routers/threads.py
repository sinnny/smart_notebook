...
from schemas import (
    ThreadCreate, ThreadList, ThreadResponse, MessageList, 
    SuccessResponse, ThreadActionRequest, Thread
)
...
async def get_threads(user_id: str = Depends(get_current_user_id)):
    try:
        # Filter by user_id
        response = supabase.table("threads").select("*").eq("user_id", user_id).order("created_at", desc=True).execute()
        return ThreadList(threads=response.data)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/threads", response_model=ThreadResponse)
async def create_thread(thread: ThreadCreate, user_id: str = Depends(get_current_user_id)):
    try:
        # Insert with user_id
...
async def toggle_bookmark_thread(action: str, req: ThreadActionRequest, user_id: str = Depends(get_current_user_id)):
    try:
