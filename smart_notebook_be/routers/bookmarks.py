...
from schemas import (
    Bookmark, BookmarkCreate, BookmarkList, Thread, ThreadList, 
    ThreadActionRequest, SuccessResponse, CreateBookmarkResponse
)
...
async def get_bookmarks(thread_id: str, user_id: str = Depends(get_current_user_id)):
    try:
        ...
        thread = supabase.table("threads").select("user_id").eq("id", thread_id).execute()
        if not thread.data:
...
async def create_bookmark(bookmark: BookmarkCreate, user_id: str = Depends(get_current_user_id)):
    try:
...
async def add_bookmarked_thread(req: ThreadActionRequest, user_id: str = Depends(get_current_user_id)):
    try:
        # Verify ownership
        thread = supabase.table("threads").select("user_id").eq("id", req.threadId).execute()
        if not thread.data:
            raise HTTPException(status_code=404, detail="Thread not found")
        if thread.data[0].get("user_id") != user_id:
            raise HTTPException(status_code=403, detail="Access denied")
...
async def remove_bookmarked_thread(req: ThreadActionRequest, user_id: str = Depends(get_current_user_id)):
    try:
